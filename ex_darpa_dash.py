...
wing_displacement = index_functions(surface_names, 'displacements_index', blinespace, 3)
wing_displacement.inverse_evaluate(para_grid, nodal_displacements)
# system_m3l_model.register_output(wing_displacement.coefficients, design_condition=cruise_condition)
...
# the motor model
motor = Motor(component=wing, motor_power_density=3.1, eta=0.75, ratio=2.0, motor_cg=np.array([4.6,0,0.2])) # 3.1
motor_mass, motor_cg, motor_inertia = motor.evaluate()
cruise_model.register_output(motor_mass)

# the fuselage mass model
fuselage = Fuselage(component=wing, fuse_mass=200, fuse_cg=np.array([4.5,0,0.2]), fuse_cd=0.4, area=0.25)
...
class TC2DB(ld.DashBuilder):
    def define(self, *args):
        ...
        def plot_2D(ax_subplot, data_dict, data_dict_history):
            ...
            ax_subplot.set_ylabel('Wing cap thickness (m)')
            # ax_subplot.set_ylim([0.0, 5.0])
            # ax_subplot.set_xlim([0.0, 5.0])
            ax_subplot.set_xlabel('Span (ft)')
            ax_subplot.set_title('Wing cap thickness vs span')
        ...
        def plot_2D(ax_subplot, data_dict, data_dict_history):
            ...
            # ax_subplot.set_xlim([0.0, 5.0])
            ax_subplot.set_xlabel('Span (ft)')
            ax_subplot.set_title('Wing stress vs span')
